"""
    The main file is adapted from the Pytorch implementation of Wide Residual Networks:
    http://arxiv.org/abs/1605.07146
    https://github.com/szagoruyko/wide-residual-networks

    2016 Sergey Zagoruyko & Shell Hu
"""
...
from models.utils import cast, data_parallel, print_tensor_dict
import torch.backends.cudnn as cudnn
import models

cudnn.benchmark = True

parser = argparse.ArgumentParser(description='Symm Wide Residual Networks')
# Model options
...
parser.add_argument('--symm_type', default='tri', type=str,
                    help='tri, evd, None')
...
def main():
    ...
    f, params, stats = getattr(models, opt.model)(opt.depth, opt.width,
                                                  num_classes, opt.symm_type)

    def create_optimizer(opt, lr):
        print('creating optimizer with lr = ', lr)
        return SGD(params.values(), lr, momentum=0.9, weight_decay=opt.weight_decay)
    ...
    # symmetric changes
    def estimate_used_parameters(n_output, n_input, kh, kw):
        assert n_output == n_input, 'can only do triu on square'
        n = n_output
        return (n * n - sum(range(n))) * kh * kw

    if opt.symm_type is not 'None':
        n_parameters = sum(estimate_used_parameters(*p.data.shape) if k.endswith('conv1') else p.numel() for k, p in params.items())
    else:
