...
def resnet(depth, width, num_classes, symm_type='None'):
    ...
    def block(x, params, stats, base, mode, stride):
        if symm_type == 'tri':
            ...
            g1 = eig_decomp_reconstruct
        elif symm_type == 'spa':
            g1 = triu_diag_triut_spatial
        elif symm_type == 'tri+spa':
            g1 = lambda y: triu_diag_triut_spatial( triu_diag_triut(y) )
        elif symm_type == 'wwt':
            g1 = w_w_transpose
        else:
            g1 = lambda y: y
        g0 = g1 if ALL_CONV_SYMM else lambda y: y

        o1 = F.relu(batch_norm(x, params, stats, base + '.bn0', mode), inplace=True)
        y = F.conv2d(o1, g0(params[base + '.conv0']), stride=stride, padding=1)
        o2 = F.relu(batch_norm(y, params, stats, base + '.bn1', mode), inplace=True)
        z = F.conv2d(o2, g1(params[base + '.conv1']), stride=1, padding=1)
